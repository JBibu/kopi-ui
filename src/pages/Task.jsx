...
function TaskInternal(props) {
  ...
  const summaryControl = useCallback((task) => {
    if (!task) return null;
  ...
  const createCounterBadge = useCallback((label, counter) => {
  ...
  const counterLevelToSortOrder = useCallback((level) => {
    switch (level) {
      case "error":
        return 30;
      case "notice":
  ...
  const sortedBadges = useMemo(() => {
    ...
    return keys.map((c) => createCounterBadge(c, task.counters[c])).filter(Boolean);
