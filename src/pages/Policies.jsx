...
export function PoliciesInternal({ navigate }) {
  ...
  const fetchSourcesWithoutSpinner = () => {
    axios
      .get("/api/v1/sources")
      .then((result) => {
        setState(prev => ({
  ...
  const isGlobalPolicy = (x) => {
    return !x.target.userName && !x.target.host && !x.target.path;
  };

  const isLocalHostPolicy = (x) => {
    return !x.target.userName && x.target.host === state.localHost && !x.target.path;
  };

  const isLocalUserPolicy = (x) => {
    return formatOwnerName(x.target) === state.localSourceName;
  };
  ...
  switch (state.selectedOwner) {
    ...
    default:
      policies = policies.filter((x) => formatOwnerName(x.target) === state.selectedOwner);
      break;
  }
