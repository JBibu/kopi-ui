...
const usePolicyEditor = (props) => {
  ...
  // Memoized API URL
  const policyURL = useCallback((currentProps) => {
  ...
  const getAndValidatePolicy = useCallback(() => {
    const removeEmpty = (l) => {
      if (!l) return l;
      return l.filter(s => s !== "");
    };

    const validateTimesOfDay = (l) => {
      for (const tod of l) {
    ...
    try {
      ...
      let policy = JSON.parse(JSON.stringify(state.policy));
      ...
      if (policy.actions) {
        if (policy.actions.beforeSnapshotRoot) {
          policy.actions.beforeSnapshotRoot = removeEmpty(policy.actions.beforeSnapshotRoot);
        }
        if (policy.actions.afterSnapshotRoot) {
          policy.actions.afterSnapshotRoot = removeEmpty(policy.actions.afterSnapshotRoot);
        }
        if (policy.actions.beforeFolder) {
          policy.actions.beforeFolder = removeEmpty(policy.actions.beforeFolder);
        }
        if (policy.actions.afterFolder) {
          policy.actions.afterFolder = removeEmpty(policy.actions.afterFolder);
        }
      }

      return policy;
    } catch (error) {
      showError(error, "Policy Validation Error");
      return null;
    }
  }, [state.policy, showError]);

  // Fetch policy from server
  const fetchPolicy = useCallback(async (currentProps) => {
    try {
  ...
  const resolvePolicy = useCallback(async (currentProps) => {
    const url = "/api/v1/policy/resolve?" + sourceQueryStringParams(currentProps);
    const policyData = getAndValidatePolicy();

    if (!policyData) {
      setState(prev => ({ ...prev, resolvedError: "Failed to resolve policy configuration" }));
      return;
    }
    ...
    try {
      ...
      setState(prev => ({ ...prev, resolvedError: error }));
    }
  }, [getAndValidatePolicy]);

  // Handle form changes
  const handlePolicyChange = useCallback((event, valueGetter = (x) => x.value) => {
...
export const PolicyEditor = (props) => {
  ...
  const policyDefinitionPoint = useCallback((p) => {
    if (!p) return "";
